...
def print_if(object_to_be_printed: object, printing_if: bool, pretty: bool = False) -> None:
    if printing_if:
        if pretty:
            pprint.pp(object_to_be_printed)
        else:
...
def save_to_csv(data: pd.DataFrame, folder_name: str, file_name: str, saving_index: bool, saving_to_csv: bool,
                printing_if: bool) -> None:
    if saving_to_csv:
        file_path_csv = os.path.join(folder_name, f'{file_name}.csv')
        data.to_csv(file_path_csv, index=saving_index)
        print_if('saved ' + file_path_csv, printing_if)


def read_from_csv(folder_name: str, file_name: str, printing_if: bool) -> pd.DataFrame:
    file_path_csv = os.path.join(folder_name, f'{file_name}.csv')
    print_if('read ' + file_path_csv, printing_if)
    return pd.read_csv(file_path_csv)


def generate_ta_id(id: int, test_on: bool, test_case_ids: list[int], printing_if: bool) -> str:
    if id == 0:
        ta_id = ''
    elif isna(id):
    ...
    if test_on:
        for case_id in test_case_ids:
            if id == case_id:
                f.print_if(f'generate_ta_id: id = {id}, ta_id = {ta_id}', printing_if)
    return ta_id
...
def anonymize_and_index(df: pd.DataFrame, random_seed: int,
                        test_on: bool, test_case_ids: list[int], printing_if: bool) -> pd.DataFrame:
    ...
    with warnings.catch_warnings():
        ...
        df['id'] = df['id'].apply(generate_ta_id,
                                  args=(test_on, test_case_ids, printing_if))  # id is the random seed for each row
